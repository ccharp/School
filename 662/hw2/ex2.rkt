...
(define-type CFWAE
  ...
  (appW (fun-name CFWAE?)(arg-expr CFWAE?))
...
(define elab-cfwae
  (lambda (expr)
    (type-case CFWAE expr
      ...
      (appW (fun-expr arg-expr)(app (elab-cfwae fun-expr)(elab-cfwae arg-expr)))
...
(test (eval-cfwae (withW 'add1 (funW 'x (plusW (idW 'x) (numW 1)))
                      (cond0W (list (appW (idW 'add1)(numW 0)) (numW 5)
                                    (numW 3) (numW 4)
                                    (numW 0) (appW (idW 'add1)(numW 2))
                                    (numW 2)))))
      (num 3))
