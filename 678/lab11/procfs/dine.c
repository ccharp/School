...
static int stop=0;
static pthread_mutex_t progress;
...
int check_for_deadlock()
{
	...
	for (i = 0; i < NUM_PHILS; i++) {
		...
		if(!init) {
			...
			sys_progress[i] = sys_time[i] - sys_time[i - NUM_PHILS - 3];
			user_progress[i] = user_time[i] - user_time[i - NUM_PHILS - 3];
