...
//OVERLOAD '=' operator
template <typename T>
const MyDlist<T> MyDlist<T>::operator =(MyDlist<T> & newList)
{	
	...
	{
		//cout << "Here123" << endl;
		push_back(newList.getNode(i)->getValue()); 
		//cout << "Here" << endl;
	}
	return *this;
}
...
MyDlist<T>::MyDlist(MyDlist<T> & oList)
{
  ...
  for(int i=0; i<oList.getSize(); i++)
    this->push_back(oList.getNode(i)->getValue());
}
...
void MyDlist<T>::push_back(T value)
{
	...
	size++;
	//cout << "size in push_back is " << size << endl;
}
